...
| Work Order 005 Task A output + device application-readiness update | `specs/reconstruct/EXTERNAL_RECORD_INTEGRATION_SPEC.md` | DRAFT_FOR_REVIEW v0.2 / APPLICATION_READINESS | AthleteTime PB/SB plus official Garmin/COROS activity and user-selected workout boundaries. Open issues 12, canonical blockers 10 by direct table recount. Application readiness is not live account linking or runtime evidence. |
...
| `EXTERNAL_RECORD_INTEGRATION_SPEC.md` | `specs/reconstruct/EXTERNAL_RECORD_INTEGRATION_SPEC.md` | DRAFT_FOR_REVIEW v0.2 / APPLICATION_READINESS | Preserves AthleteTime PB/SB boundaries and adds official Garmin/COROS application-readiness, pending-confirmation activity staging, token/privacy limits, revocation, and future user-selected workout boundaries. Public linking and production ingestion remain blocked. Not canonical or runtime evidence. |
...
| `PLAN_BACKUP_PUBLIC_PROFILE_AND_SHARING_SPEC.md` | `specs/reconstruct/PLAN_BACKUP_PUBLIC_PROFILE_AND_SHARING_SPEC.md` | RECONSTRUCTED_DRAFT_FOR_REVIEW / IMPLEMENTATION_ALIGNED | Separates private validated-plan backup, opt-in public profiles, and bounded friend-share cards. Detailed prescriptions, journals, health signals, and memo text are excluded from public projection. Not canonical or runtime evidence. |
| `ACCOUNT_AUTHENTICATION_AND_IDENTITY_LINKING_SPEC.md` | `specs/reconstruct/ACCOUNT_AUTHENTICATION_AND_IDENTITY_LINKING_SPEC.md` | DRAFT_FOR_REVIEW / OWNER-APPROVED IMPLEMENTATION DIRECTION | Current first-wave Supabase Auth contract for Kakao, Google, email OTP, pre-auth age/legal checks, and under-14 local-only use. Not canonical, provider runtime evidence, or public release approval. |
| `FEDERATED_ACCOUNT_SSO_CONTRACT.md` | `specs/reconstruct/FEDERATED_ACCOUNT_SSO_CONTRACT.md` | DRAFT_FOR_REVIEW / FUTURE_DEFERRED_OPTIONAL_PROVIDER | Work Order 006 Task C draft retained for a future optional AthleteTime identity link. It is not the current identity root, a first-release blocker, canonical, or runtime evidence. |
| `DOUBLE_SESSION_BETA_SAFETY_CONTRACT.md` | `specs/reconstruct/DOUBLE_SESSION_BETA_SAFETY_CONTRACT.md` | DRAFT_FOR_REVIEW (v0.3, 2026-08-12) | Bounded local-beta AM/PM session contract. A second session requires explicit athlete consent (`RECOVERY_PM_ALLOWED`); when directly chosen, every available training day receives two distinct slots. **v0.3 supersedes the previous PM recovery-session count cap for that full user-selected schedule**, without opening individual numeric prescription, automatic same-day double quality, catch-up sessions, or D9 bypass. PM may be rest, easy, or quality, with rest/light as the recommended default (OD-SLOT-1/2/7). Same-day quality remains **one by default**; two require explicit designation plus the disclosure-and-edit flow of `DSB-INV-009`, which the current build does not yet provide (backlog `B-17`). Four open issues, three of them canonical blockers. **No CI job validates any `DSB-INV-*` rule.** Read §0 and the §10 change history before citing it. Not canonical or runtime evidence. |
| `ADVISORY_SESSION_EXAMPLE_RECOMMENDER_CONTRACT.md` | `specs/reconstruct/ADVISORY_SESSION_EXAMPLE_RECOMMENDER_CONTRACT.md` | DRAFT_FOR_REVIEW | Research-example advisory contract. It filters direct/adapted source tiers before ranking, returns exactly two or three examples only in isolated eligible fixtures, and permits only a twice-confirmed non-executable personal draft. The current 30-entry catalog has zero runtime candidates. Not canonical, activated, or runtime behavior. |
| `TRAINING_TERMINOLOGY_AND_EXPLANATION_SPEC.md` | `specs/reconstruct/TRAINING_TERMINOLOGY_AND_EXPLANATION_SPEC.md` | DRAFT_FOR_REVIEW | Separates schedule roles, plan intents, metabolic pathways, fuel context, lactate response, and athlete-reported metrics. Requires Korean-first labels and progressive explanations without changing stored enums, rule semantics, plans, or safety state. |
...
19. Review `specs/reconstruct/EXTERNAL_RECORD_INTEGRATION_SPEC.md` v0.2 with `OWNER_DECISION_DEVICE_INTEGRATION_APPLICATION_2026_08_29.md`; application-readiness work is allowed, but AthleteTime API work, public Garmin/COROS linking, provider token storage, production ingestion, analysis adoption, and outbound workout sync remain blocked by their named gates.
