...
for (const row of responseRows) {
  failUnless(
    /^\| CR-(?:0[1-9]|[12][0-9]|30) \| APPROVE(?:_WITH_[A-Z_]+)?_RECORDED_PENDING_INDEPENDENT_REVIEW \| [^|]+ \|$/u.test(row),
    `response row must record a pending-review approval with a rule sentence: ${row}`,
  );
}
...
const alignedMarkers = [
  ...
  ["fixtures", "visible text marker; not a star-only marker"],
...
for (const marker of [
  ...
  "conservative rest",
  "A canonical day-boundary definition remains open",
