...
test("requires receipt tokens instead of literal artifact PR URLs", async () => {
  await withFixture("valid-terra", async (directory) => {
    assert.doesNotThrow(() => validateArtifact(directory, "fable"));
    assert.doesNotThrow(() => validateArtifact(directory, "terra"));
  });

  await withFixture("valid-terra", async (directory) => {
    await replaceIn(directory, "reports/review/WO017_FABLE_UX_FLOW.md", "pr_url: RECORDED_IN_GITHUB_RECEIPT", "pr_url: https://github.com/hojune0330/TRAINORACLE/pull/171");
    await replaceIn(directory, "reports/review/WO017_TERRA_BINDING_MATRIX.md", "pr_url: RECORDED_IN_GITHUB_RECEIPT", "pr_url: https://github.com/hojune0330/TRAINORACLE/pull/172");
    assert.throws(() => validateArtifact(directory, "fable"), (error) => error instanceof Wo017Error && error.code === "LITERAL_WO017_PR_URL");
    assert.throws(() => validateArtifact(directory, "terra"), (error) => error instanceof Wo017Error && error.code === "LITERAL_WO017_PR_URL");
  });
});
