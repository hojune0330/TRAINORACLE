...
test("Given an altered competition marker, when the walkthrough validates, then it fails closed", async () => {
  const result = await validateWith({
    relativePath: packagePaths[0],
    from: "visible text marker; not a star-only marker",
    to: "visible text marker; star-only marker is allowed",
  });

  assert.notEqual(result.status, 0);
  assert.match(result.stderr, /fixtures missing marker: visible text marker; not a star-only marker/u);
});
