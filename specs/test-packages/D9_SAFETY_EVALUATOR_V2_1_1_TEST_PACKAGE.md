# D9_SAFETY_EVALUATOR_V2_1_1_TEST_PACKAGE.md

```yaml
package_id: D9_SAFETY_EVALUATOR_V2_1_1_TEST_PACKAGE
target_rule_ref: RULE_SPEC_D1_D9.D-9
candidate_version: v2.1.1
status: READY_FOR_LOCAL_TEST
canonical_evidence: false
validation_source_required: actual_vitest_output_only
```

## 목적

이 패키지는 D-9 안전 하드스톱 evaluator 후보를 로컬에서 검증하기 위한 실행 가능한 Vitest 테스트 파일을 포함한다.

- 이 엔진은 의학적 진단을 하지 않는다.
- ACTIVE / UNKNOWN은 계획 생성을 막는다.
- ADVISORY는 계획 생성을 막지 않지만 reasonCode를 남긴다.
- 선수 자유서술 원문은 운영 신호에 저장하지 않는다.
- 실제 canonical evidence는 로컬 Vitest PASS 이후에만 인정한다.

## 실행 방법

아래 TypeScript 코드블록을 `d9-safety-evaluator.vitest.ts`로 저장한 뒤 실행한다.

```bash
npm i -D vitest typescript
npx vitest run d9-safety-evaluator.vitest.ts --reporter=verbose
```

## TypeScript / Vitest 파일
...
실패하면 전체 파일을 다시 보내지 말고 아래 형식만 보낸다.
...
운영 신호에는 자유서술 원문 또는 clause를 저장하지 않는다.

```yaml
production_signal_should_store:
  - ruleRef
  - storedStatus
