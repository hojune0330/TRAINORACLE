...
function requireGithubReceiptToken(text, label) {
  if (/^pr_url: https:\/\/github\.com\/hojune0330\/TRAINORACLE\/pull\/\d+$/mu.test(text)) {
    fail("LITERAL_WO017_PR_URL", `${label} PR URL belongs in the GitHub receipt`);
  }
  if (!/^pr_url: RECORDED_IN_GITHUB_RECEIPT$/mu.test(text)) {
    fail("MISSING_WO017_PR_LINK", `${label} receipt token missing`);
  }
}
...
function validateOrder(root) {
  ...
  for (const marker of [
    ...
    "[WO017] Issue replacement evidence track",
    "[WO017] Carry forward Fable onboarding UX flow",
    "[WO017] Bind replacement UX to current contracts",
    "[WO017] Record replacement Sol delta advisory",
  ]) requireText(text, marker);
...
function validateFable(root) {
  ...
  requireGithubReceiptToken(text, "Fable");
