...
| **규칙 코드** | 영어 + 모노 | 단축 참조 | `R-6, LEGACY_PHASE_D.D-5, CYCLE_DAY.D-5, T-2, Q3` |
...
### 0.5 Canonical Safety Statements (S-01 ~ S-04)

> **이 4문장은 TRAINORACLE 프로젝트의 최고 우선 안전 약속입니다.**
>
> 다른 어떤 문서·코드·UI도 본 4문장과 모순될 수 없습니다.
> 충돌 발생 시: **최신 PATCH_NOTES → 채택된 ADR → 본 §0.5 → 정본 문서 → 화면 사양** 순으로 우선합니다.
>
> 이 4문장이 바뀌어야 할 때는 반드시 PATCH_NOTE를 작성하고, 본 §0.5를 갱신한 뒤, 영향 받는 docs를 동기화합니다.

---

#### **S-01. AI Coach Jang UI는 항상 유지됩니다**

AI Coach Jang 페르소나의 채팅·코멘트·브리핑 UI는 Free·Beta·Pro 모든 단계에서 동일하게 노출됩니다.
**UI의 존재가 외부 LLM 호출을 의미하지 않습니다.**
응답을 생성하는 레이어(Rule Engine + Template / LLM Explainer)는 단계에 따라 다르지만, 사용자가 보는 화면은 일관됩니다.

- 시각적 동일성: AI Chat Bubble, Daily Brief, AI Inbox 등 컴포넌트는 단계와 무관하게 동일.
- 차이는 **응답 출처(`source_layer` 필드)**로만 표시: `template-only` / `rule-engine` / `llm-explainer`.
- 관련 ADR: ADR-008.

---

#### **S-02. Free/Beta 단계에서는 외부 LLM API 호출이 금지됩니다**

사용자 데이터를 외부 LLM(Anthropic·OpenAI·Google 등)에 전송하지 않습니다.
모든 사용자 노출 응답은 **Rule Engine + Knowledge Files + Template Feedback** 으로 생성합니다.

- 위반 시 즉시 응답 폐기 + PATCH_NOTE 자동 발행.
- Knowledge Files = `docs/` 정본 문서 + GLOSSARY + PHILOSOPHY (로컬 검색만).
- Pro 단계에서 LLM이 활성화되더라도, 사용자 명시 동의 + 옵트아웃 토글 필수.
- 관련 ADR: ADR-002.
- 관련 PATCH: `docs/PATCH_NOTES/2026-04-29-no-external-llm-before-pro.md`.

---

#### **S-03. LLM은 결정 권한이 없습니다 (Explainer Only)**

Pro 단계에서 LLM이 활성화되어도, **모든 의사결정은 Rule Engine(Layer 1)에서만** 이루어집니다.
LLM(Layer 2)은 Layer 1이 산출한 `Verdict + Evidence`를 자연어로 재진술하는 **Explainer 역할**에 한정됩니다.

- LLM이 새 verdict·새 수치·새 권고를 생성하면 Output Validator에서 차단.
- LLM 응답이 Layer 1과 모순되면 즉시 폐기, Layer 1 raw output만 표시.
- 의료 진단·처방 어휘 차단(blocklist).
- 관련 ADR: ADR-003.
- 관련 PATCH: `docs/PATCH_NOTES/2026-04-29-rule-engine-before-llm.md`.

---

#### **S-04. 분석은 단 하나의 Primary Device 데이터로만 수행합니다**

사용자가 여러 장치를 사용해도, 9 Rules 검증·Classification·PB 갱신 등 **모든 분석은 Primary Device 한 대의 데이터로만** 진행합니다.

- Settings에서 사용자가 단일 선택(라디오 버튼).
- 다른 장치 데이터는 Personal Archive에 보존만 가능, 분석 입력에서 제외.
- 다중 장치 지원은 RFC를 통해서만 도입 가능 (MVP 단계 거부).
- 관련 ADR: ADR-001.
- 관련 PATCH: `docs/PATCH_NOTES/2026-04-29-primary-device-only.md`.

---

#### **§0.5 인용 의무**

다음 문서는 §0에 본 §0.5를 한 줄 이상 인용해야 합니다 (충돌 방지):

- `docs/02_AI_STRATEGY.md` (S-01, S-02, S-03 직접 영향)
- `docs/06_VALIDATION_AND_SAFEGUARDS.md` (S-02, S-03, S-04 직접 영향)
- `docs/07_FEEDBACK_SYSTEM.md` (S-01, S-02 직접 영향)
...
- 사용 맥락: 9.5-Cycle 안에서 계획된 핵심 훈련 (보통 `CYCLE_DAY.D-2`, `CYCLE_DAY.D-5`, `CYCLE_DAY.D-8` 등)
...
- 사용 맥락: 대회 CYCLE_DAY.D-7 ~ CYCLE_DAY.D-1
- 표기: 한국어 본문에서는 `taper` 그대로 또는 "테이퍼"
- 예: "대회 `CYCLE_DAY.D-5` · `taper` 강도"
...
### D-rule Namespace Notice

현재 TrainOracle SPEC 레이어에서는 `D-1`~`D-9`를 단독으로 사용하지 않는다.

- `RULE_SPEC_D1_D9.D-*`: 현재 app/spec validation rule.
- `LEGACY_PHASE_D.D-*`: 기존 workflow Phase D 훈련검증 항목.
- `CYCLE_DAY.D-*`: 사이클 또는 대회 기준 날짜 라벨. rule id가 아니다.

새 SPEC 문서에서는 bare `D-1`, `D-9` 표기를 금지한다.  
충돌 시 새 SPEC 작성에서는 `RULE_SPEC_D1_D9`가 우선한다.
...
- **D-day 기준**: 사이클 시작일을 CYCLE_DAY.D-1, 끝을 CYCLE_DAY.D-9.5
- **예**: "Cycle 7 · Day 5/9.5 · 다음 MAIN CYCLE_DAY.D-8"
...
> Status: `LEGACY_WORKFLOW_TERM`  
> 이 항목은 현재 SPEC 레이어의 `RULE_SPEC_D1_D9.D-*` rule id가 아니다.

### 2.3 Cycle Day labels

- **namespace**: `CYCLE_DAY`
- **풀이**: 사이클 또는 대회 기준 날짜 라벨. rule id가 아니다.
- **표기**: `CYCLE_DAY.D-5`, `CYCLE_DAY.D-9.5`
- **예**: "대회 `CYCLE_DAY.D-5` 테이퍼 강도"
- **주의**: `RULE_SPEC_D1_D9.D-*` 또는 `LEGACY_PHASE_D.D-*` rule id와 다르다.
...
- **Cycle day labels**: `CYCLE_DAY.D-1`–`CYCLE_DAY.D-9.5`  사이클/대회 기준 날짜 라벨이며 rule id가 아니다.
- **Trigger rules**: `T-2` 등
...
| 약어 코드 (R-N, LEGACY_PHASE_D.D-N, CYCLE_DAY.D-N) | **영어 + 모노** |
| 개발 내부 문서 | **영어** OK |
| 사용자 노출 라벨 | **한국어 우선** |
...
| 2026-04-29 | **§0.5 Canonical Safety Statements (S-01~S-04) 추가** | 기존 §0.5(격식)는 §0.6으로 밀림. 02/06/07 §0 인용 의무 명시. PATCH `2026-04-29-safety-statement-introduced.md` 발행 |
