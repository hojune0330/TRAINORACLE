...
> **🔴 2026-08-06 통합 지시서 발행 — 아래 6건은 하나의 연속 작업으로 묶였다.**
> 오너 지시(`"불필요한 중간 리뷰 없이 최대한 다 진행하게 하는 방안"`)에 따라
> B-18·B-14·B-15·B-16·B-11·B-13 을 **S-1~S-6** 단계로 재편해
> [`WORK_ORDER_SLOT_INTENSITY_FULL_RUN.md`](WORK_ORDER_SLOT_INTENSITY_FULL_RUN.md) 한 건으로 발행했다.
> 중간 리뷰 없이 S-6까지 진행하고 **PR 1건**으로 끝에서 한 번만 리뷰한다.
>
> | 단계 | 백로그 항목 | 상세 좌표 명세 |
> |---|---|---|
> | S-1 훈련 시간대 질문 | B-18 | `WORK_ORDER_TRAINING_TIME_QUESTION_C3A0.md` (흡수됨) |
> | S-2 생성기 오전 고정 해제 | B-14 | `WORK_ORDER_PM_QUALITY_GENERATOR_C3A.md` (흡수됨) |
> | S-3 저장 관문 개정 | B-15 | 통합 지시서 §4 |
> | S-4 화면 문구 정정 | B-16 | 통합 지시서 §4.8 |
> | S-5 기록 상세 접근성 | B-11 | 통합 지시서 §5 |
> | S-6 leaf 좌표 유일성 | B-13 | 통합 지시서 §6 |
>
> **범위 밖(같은 PR에 넣지 않는다):** B-17(OD-SLOT-6 수정·확정 플로우), B-12(계정 잠금 해제 화면).
> 둘 다 오너의 설계 결정이 먼저 필요하다 — 통합 지시서 §12.

> **🔴 2026-08-06 전역 감사 지시서 별도 발행 (읽기 전용, 위 S-1~S-6과 무관하게 병행 가능).**
> 오너 지시(`"더 광범위한 스펙 조사나 문서와 작업에 대한 검증 작업을 지시하는 지시서를 발행"`)에 따라
> [`WORK_ORDER_DEEPSEEK_SPEC_WIDE_AUDIT.md`](WORK_ORDER_DEEPSEEK_SPEC_WIDE_AUDIT.md)를 발행했다.
> 수행자는 **딥시크**, 모드는 **읽기 전용**(코드·스펙·CI 수정 금지, 커밋 금지),
> 산출물은 `reports/review/deepseek-audit/` 아래 보고서 24건뿐이다.
>
> 이 지시서를 쓰기 위한 사전 실측에서 **아직 아무 백로그 항목도 되지 않은 발견 3건**이 나왔다:
>
> | ID | 발견 | 실측값 |
> |---|---|---|
> | F-1 | 검증기 55개 중 CI 등록은 14개. **41개가 고아**이며 지금 통과하는지 실패하는지 미확인 | 55 / 14 / 41 |
> | F-2 | **`validate-latest-owner-decision.mjs`가 존재하고 통과하며 `conflicts=12`를 보고하는데 CI에 없다.** "최신 오너 결정이 이긴다"는 최상위 규칙의 검증기가 방치돼 있다 | `conflicts=12 latest_decision=governs runtime=false` |
> | F-3 | 문서가 참조하는 유니크 경로 중 **약 12%가 저장소에서 해석되지 않는다** (스코프 의존 참고치 — 독립 실측: b91=791/89 · d9dc=802/95 · a29=775/87. v1.0의 "795/112"는 미재현 → 지시서 v1.1에서 격하) | 791/89 · 802/95 |
>
> **아직 백로그 항목(B-NN)으로 승격하지 않았다.** 감사 보고서가 각 발견의 위험도를 확정한 뒤,
> 오너가 우선순위를 정하면 그때 승격한다. 감사자가 스스로 승격하지 않는다.
...
- **2026-08-16 진행:** `reports/review/V2_SEED_05_ACTIVATION_REVIEW_PACKET.md`에 첫 비스프린트 후보의 출처 재확인, 제안 범위, 11개 차단 항목, D9→RVE→Safety Gate 상속 계약과 4개 사람 검토 자리를 모았다. 패킷 상태는 `REVIEW_INPUT_PACKET_READY_RUNTIME_GATE_BLOCKED`이며 활성화 권한은 계속 없다.
- **런타임 경계:** 승인 레지스트리는 0개다. 현재 승인 자료형은 적격 검토자·검토 대상 내용·실재 구성요소·서버 기반 연령/보호자 권한을 증명하지 못하므로, 이를 보강하기 전에는 승인 레코드를 추가하지 않는다.
...
### B-11 🟠 §4-1 LogDetail 접근성 — B-01이 완료로 표시했지만 실측 1건 (→ **S-5**, 통합 지시서 발행됨)
...
- **🔴 2026-08-06 재실측 (main `f2d7805`):** `LogDetail.tsx`는 262행이고 **`<button>`이 파일 전체에 단 1개**(`:116-128` 되돌리기, `data-testid="delete-undo-button"`)뿐이다. **§4-1이 이름을 붙이라고 지목한 수정·삭제·뒤로·메모 열기 버튼은 이 파일에 존재하지 않는다.** 따라서 이 항목은 "aria-label을 추가하는 일"이 아니라 먼저 **대상 인벤토리를 실측하는 일**이다. 없는 버튼을 새로 만들어 붙이는 것은 금지(통합 지시서 §9 중단 조건 8번)
...
- **작업지시서:** [`WORK_ORDER_SLOT_INTENSITY_FULL_RUN.md`](WORK_ORDER_SLOT_INTENSITY_FULL_RUN.md) §5 (S-5)

### B-12 🟠 Q4 계정 잠금 해제 화면 — 도메인 함수는 있고 부르는 곳이 없다 (통합 지시서 **범위 밖**)
...
### B-13 🟠 leaf `sessions` 배열의 (day,slot) 유일성 refine 부재 — B-02에서 분리 (→ **S-6**, 통합 지시서 발행됨)
...
### B-18 🔴 S-1 훈련 시간대 질문 없음 — 생성기가 오후를 고를 근거가 없다 (통합 지시서 발행됨)
...
- **상태:** ❌ 미착수 — 통합 지시서 [`WORK_ORDER_SLOT_INTENSITY_FULL_RUN.md`](WORK_ORDER_SLOT_INTENSITY_FULL_RUN.md) **S-1**로 발행 (`f2d7805`). 상세 좌표는 `WORK_ORDER_TRAINING_TIME_QUESTION_C3A0.md`(`cee1445`, `ABSORBED_INTO_FULL_RUN`)가 유지
...
### B-14 🔴 S-2 생성기 오전 고정 — 오너 결정 미반영 (통합 지시서 발행됨, S-1 선행 필요)
...
- **상태:** ❌ 미착수 — 통합 지시서 **S-2**. **S-1(B-18) 커밋 전에는 착수 불가** (`profile.trainingTimePreference`가 없어 타입 오류). **S-3과 같은 PR에 실려야 한다** — 단독 병합 시 생성된 계획이 저장 관문에서 거부된다(결정 문서 §4.9)
- **실측 근거 (main `e639eae`):**
...
### B-15 🟠 S-3 저장 관문 — PM 제약이 오너 결정과 충돌 (C-4·C-5·**C-8·C-9**)
- **출처:** 결정 문서 C-4·C-5 · §4.3 · §5 단계 4
- **상태:** ❌ 미착수 — 통합 지시서 **S-3**. S-2 커밋 다음에 오지만 **같은 PR 안**이다(별도 PR로 나누지 않는다)
- **실측 근거 (main `e639eae`):**
...
- **🔴 2026-08-06 저장 관문 실측으로 발굴한 신규 충돌 2건** (통과 기대 대조군을 포함한 유효 측정, 결정 문서 §4.9):
  - **C-8** `plan-beta-schema.ts:92-94` — 동의 검사가 "PM 슬롯"에 걸려 있어 **"두 번째 세션"과 구분하지 못한다.** 그래서 저녁에 **하루 한 번만** 훈련하는 사용자의 오후 단일 세션도 거부된다. 검사 주체를 "PM 슬롯"에서 "하루 2세션"으로 옮겨야 한다
  - **C-9** `plan-beta-schema.ts:95-100` — 통증이 있어 `CONSERVATIVE` 후보를 받은 저녁 훈련자는 **오후 세션 자체를 가질 수 없다.** 이 검사는 삭제 대상
- **🔴 왜 S-2와 같은 PR인가:** S-2(생성기)만 병합하면 생성된 오후 계획이 이 관문에서 거부되고 `savePlanBetaState`가 `PLAN_STORAGE_WRITE_FAILED`를 돌려준다(`plan-beta-store.ts:54-71`). 사용자는 `"계획을 이 기기에 저장하지 못했어요"`를 본다(`PlanBeta.tsx:212`). `main` push는 자동 배포이므로 **깨진 상태가 그대로 나간다.** 단계별 커밋은 분리하되 PR은 하나로 한다
- **주의:** 검사를 **없애는 것이 아니라 대상을 바꾸는 것**이다. "PM은 회복만" → "PM은 무엇이든 되지만 하루 고강도 2회는 명시 지정 필요" (결정 문서 §4.3)
- **완료 조건:** 통합 지시서 §4.6 G-1~G-10. 핵은 **G-10** — S-2 생성기를 직접 호출해 그 결과를 그대로 저장 관문에 넣어 통과함을 증명한다(손으로 만든 fixture로 대신하지 않는다) + 하루 2회 고강도가 명시 지정 없이는 거부됨을 고정 + 결함 주입 비공허성
- **🔴 fixture 유효성 원장:** 저장 관문 테스트에는 **반드시 통과 기대 대조군을 1건 이상 넣는다.** 전부 거부 기대인 테스트 파일은 아무것도 증명하지 못한다 — 말바를진 fixture는 엉뚱한 이유로 거부된다(2026-08-06에 실제로 당함: `kind`는 `"ACTIVE_PLAN"`이 아니고 `"BETA_ACTIVE_PLAN_SNAPSHOT"`이다)

### B-16 🟠 S-4 화면 문구 — 사용자에게 하는 거짓 약속
- **출처:** 결정 문서 C-6 · §6 · OD-SLOT-7
- **상태:** ❌ 미착수 — 통합 지시서 **S-4**. **S-3 커밋 후에만 착수** (런타임보다 먼저 고치면 거짓말 방향만 바뀐다)
- **⚠️ 이 단계가 깨뜨리는 테스트:** `app/e2e/launch-ready.spec.ts:97` 이 `/오후 회복/` 가시성을 단정한다. **skip 하지 말고 새 문구에 맞춰 고칠 것**
...
### B-17 🟠 OD-SLOT-6 계획 수정·최종확정 플로우 — 신규 화면 (통합 지시서 **범위 밖**)
