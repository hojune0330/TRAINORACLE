...
2. 🔴 **철회됨 (2026-08-06 2차 리뷰).** 이 항목은 원래 이렇게 적혀 있었다:
   ...
   **이대로 하면 훈련량이 조용히 늘어난다.** 이유는 단순하다 — 위 `:208`의
   짝 세션은 **`RECOVERY_INTENT`** 로 만들어진다. 그래서
   `recoverySecondSessionDays()`가 만드는 회복 PM과 **같은 상한**
   (`DSB-INV-005`: PM `RECOVERY_INTENT` 세션, 9·9.5·10일 프레임 **2건**)
   안에 함께 들어간다. 두 장치가 같은 예산을 쓰면서 서로를 모른다.

   **3차 실측 (2026-08-06, `generatePlanFromDraft(draft, "NO_KNOWN_RISK")`,
   BALANCED, 9.5일 프레임, 420조합 전수):**
   PM 회복 세션이 현재 2건 → 고강도 1일이면 **3건**, 고강도 2일이면 **4건**.
   **420조합 중 300건(71%)이 상한 2를 넘는다.**
   최악(`MIDDLE_DISTANCE`/`DEVELOPING`/4일/`LT_INTENT`):
   6회 220분 → 8회 270분(+23%), 하루2회 날 2일 → 4일.

   ⚠️ **이 숫자는 두 번 잘못 보고됐다가 3차에 바로잡힌 것이다.**
   1차는 필드명을 `sessionDay`로 잘못 읽었고(실제는 **`s.day`**),
   2차는 짝 세션 의도를 `BASE_INTENT`로 착각해 *"상한 밖이라 위반 없음"* 이라고
   잘못 정정했다. **실제로는 `RECOVERY_INTENT`이고 위반이 사실이다.**
   `DSB-INV-005`는 v0.2에서도 **은퇴하지 않았다** — 오너 결정 OD-SLOT-8은
   "하루 두 번을 허용하는가"에 대한 판단이고, "회복 세션 건수 상한"은
   오너가 판단한 적이 없는 별개 축이다.

   **대신 이렇게 해라:** 두 장치가 **하나의 예산**을 나눠 쓴다. 짝 세션 우선,
   남은 몫만 회복 PM. **정확한 규칙·실측표·고정 테스트(G-20~G-24)·실측 재현
   절차는 `WORK_ORDER_SLOT_INTENSITY_FULL_RUN.md` §3.1에 있다. 그걸 따라라.**
   (짝 세션을 `BASE_INTENT`로 바꿔 상한을 우회하는 길도 있지만 **임의로 고르지
   마라** — 고강도 날에 RPE 3-4를 붙이는 것이라 오히려 부담이 커진다.
   FULL_RUN §3.1의 「대안 검토」와 §9-14를 읽어라.)
...
| 🔴 **짝 세션과 회복 PM에 공유 예산을 안 씌움** | PM 회복 2건→3~4건. **420조합 중 300건(71%)이 `DSB-INV-005` 상한 초과.** **FULL_RUN §3.1** |
| 🔴 짝 세션이 `DSB-INV-005` 상한 밖이라고 가정 | `:208`이 **`RECOVERY_INTENT`** 라서 상한 안에 들어간다. 이 착각이 71% 초과를 놓치게 만든다 |
| 🔴 실측할 때 `s.sessionDay`·`out.candidates`·`c.candidateKind`를 읽음 | 각각 **`s.day`·`out.generated.candidates`·`c.kind`** 다. 틀리면 숫자가 조용히 0이 되고 **테스트는 통과한다.** FULL_RUN §3.1 재현 절차 |
