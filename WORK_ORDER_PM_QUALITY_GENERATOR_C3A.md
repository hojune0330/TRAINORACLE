...
2. 🔴 **철회됨 (2026-08-06 2차 리뷰).** 이 항목은 원래 이렇게 적혀 있었다:
   ...
   **이대로 하면 훈련량이 조용히 늘어난다.** 실측: 9.5일 프레임 BALANCED에서
   하루 2회 날이 **2일 → 4일**, 420개 조합 중 **300건(71%)이 사양 상한 초과**.
   `DSB-INV-005`는 9·10일 프레임 상한을 **2건**으로 정하고 있고, 어떤 오너 결정도
   이 상한을 폐기하지 않았다.

   **대신 이렇게 해라:** 두 장치의 합에 프레임 상한을 씌운다.
   **정확한 규칙·실측표·고정 테스트(G-20~G-23)는
   `WORK_ORDER_SLOT_INTENSITY_FULL_RUN.md` §3.1에 있다. 그걸 따라라.**
...
| 🔴 **프레임 훈련량 상한을 안 씌움** | 하루 2회 날 2일→4일. `DSB-INV-005` 초과. **FULL_RUN §3.1** |
