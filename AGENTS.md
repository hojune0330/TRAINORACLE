...
4. **"CI 초록색"을 "검증됨"으로 읽지 않는다.** §8의 검사 수치는 2026-08-06
   시점의 관측이다. 현재 파일과 실행 로그로 해당 규칙의 실제 검증 범위를 확인한다.

---

## 1. 왜 이 파일이 존재하는가

당시 조사에서 문서가 코드의 두 배가 넘었다(277 : 128, 현재 수치 아님). 그 문서들은 참고
...
| `decoration-collections.ts`, `decoration-collection-docs.ts`, `public/collections/`, `public/legal/open-source.html` | [`docs/handoff/decoration-collections.md`](./docs/handoff/decoration-collections.md) — 컬렉션 추가/종료 절차, 획득 경로, 문서 자동 생성(`npm run collection-docs`), e2e 의존 문자열 | ✅ 활성 |
...
```yaml
몸상태_이상시: 계획_생성_차단
일지_원문_서버전송: North_Star_3_1의_승인된_계정보관경로만_예외
일지_원문_모델컨텍스트: 금지
60m미만_스프린트_페이스환산: 금지
종목간_환산: 오너승인_없이_금지
...
## 8. 🔴 기계 검증의 사각지대를 먼저 알아라

**"CI가 초록색이다"를 "검증됐다"로 읽지 마라.** 2026-08-06 실측:

| 사실 | 실측값 | 뜻 |
|---|---|---|
| `specs/test-packages/` 검증기 | 55개 | |
| 그중 CI가 실제로 부르는 것 | **14개** | 41개는 **고아** — 지금 통과하는지조차 미확인 |
| `DSB-INV-*` 규칙의 기계 검증 | **0건** | 하루 두 번 훈련 계약 전체가 문서에만 있다 |
| 문서가 참조하는 유니크 경로 | 실측 약 **12% 해석 실패** (스코프 의존: d9dc=802/95 · b91=791/89) | "이 파일을 읽어라"를 따를 수 없는 지시가 있다 |
| `specs/` 문서의 `status:` 값 | **11종**(reconstruct+active 2폴더), 18개 문서는 status 없음 · 스코프 전체(D-05)로는 18종 | 통제 어휘가 없어 기계가 승인 여부를 알 수 없다 |
| 문서 식별자 규약 | `spec_id`(19) 와 `doc_id`(12) **공존** | 어느 쪽이 규약인지 정의 문서 미발견 |

2026-09-02 정합성 확인: 위 표는 과거 감사 기록이며 현재 부재의 증거가 아니다.
현재 AM/PM 생성·저장과 상세 처방 관련 테스트가 존재한다. 이것을 전체
`DSB-INV-*` 조항의 직접 검증 완료와 동일시하지도 않는다. 최신 경계는
[`DOUBLE_SESSION_BETA_SAFETY_CONTRACT.md`](specs/reconstruct/DOUBLE_SESSION_BETA_SAFETY_CONTRACT.md)
§4의 별도 채택 상세 템플릿 예외와
[`현재 구현 감사`](reports/review/SPEC_TO_IMPLEMENTATION_GAP_AUDIT_2026-09-02.md)를 읽는다.
기본 RPE 처방 원칙을 이유로 이미 채택된 네 상세 템플릿을 다시 막지 않으며,
반대로 AM/PM 슬롯만으로 새 MAIN·추가 QUALITY 권한을 만들지 않는다.

**따라서 스펙 문서의 규칙을 근거로 코드를 짤 때는, 그 규칙이 기계로 강제되는지
직접 확인해야 한다.** 확인 방법과 전체 조사 지시는
[`WORK_ORDER_DEEPSEEK_SPEC_WIDE_AUDIT.md`](WORK_ORDER_DEEPSEEK_SPEC_WIDE_AUDIT.md)에 있다.
그 지시서의 **§5 함정표(T-1~T-18)는 실제로 선행 작업자가 밟은 함정 모음**이므로
훈련 코드나 스펙을 만지기 전에 읽으면 같은 실수를 피할 수 있다.

특히 그중 3개는 **테스트가 통과하는데도 아무것도 검증하지 않게 만드는** 함정이다:

- `npx tsc`는 `impl/node_modules` 없이 실행하면 `tsc@2.0.4`를 내려받아 **에러 없이
  0으로 종료한다.** 반드시 `./node_modules/.bin/tsc --noEmit` (§5의 T-1)
- 세션 필드는 `day`이지 `sessionDay`가 아니다. 잘못 읽으면 `undefined`가 되고
  **집계가 쓰레기인데 테스트는 통과한다** (T-5)
- 모든 케이스가 "거부 기대"인 관문 테스트는 스키마가 전부 거부하도록 망가져도
  통과한다. **반드시 통과해야 하는 대조군 1건을 넣어라** (T-10, §5 참조)
