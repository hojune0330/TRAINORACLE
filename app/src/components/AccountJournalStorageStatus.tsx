...
export function AccountJournalStorageStatus() {
  ...
  const message = status === "LOADING" ? "계정의 일지를 불러오고 있어요." : status === "PENDING"
  ...
  const state = status === "REJECTED" || status === "FAILED" ? "error" : status === "CONFLICT" ? "conflict" : "pending"
  return <div className="account-storage-status" data-status={status}>
    <div className="account-storage-status__summary">
      {message !== null && (
        <p className="account-panel__status" data-state={state} role="status" aria-live="polite">
          {message}
        </p>
      )}
      {status === "FAILED" && <button className="account-storage-status__retry" type="button" aria-label="계정 일지 다시 불러오기" title="계정 일지 다시 불러오기" onClick={() => void hydrateAccountJournalRecords()}><RefreshCw aria-hidden="true" size={16} /></button>}
    </div>
