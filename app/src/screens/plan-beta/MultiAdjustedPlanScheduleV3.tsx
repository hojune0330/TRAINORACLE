...
export function MultiAdjustedPlanScheduleV3({ loaded, readEvidence, onStoredChange, onWritePlannedSessionLog, returnToSession, onImportPlan, onPrepareNext }: {
  ...
  return <section className="plan-active adjusted-plan-schedule" aria-labelledby="multi-adjusted-title">
    ...
    <details><summary>저장과 이용 안내</summary><p>현재 이 기기에 저장된 계획이에요.</p>
      <MultiPlanCloudControlsV3 fingerprint={loaded.state.contentFingerprint} readEvidence={readEvidence} />
      ...
      <button type="button" disabled={exporting} aria-busy={exporting} onClick={async () => {
        ...
        try {
          ...
          if (!current()) return
          const result = exportMultiAdjustedPlanBackupV3(loaded.state.contentFingerprint, evidence)
          if (!current()) return
          ...
          link.href = url; link.download = `trainoracle-multi-plan-${todayISO()}.json`
          if (!current()) return
          link.click()
