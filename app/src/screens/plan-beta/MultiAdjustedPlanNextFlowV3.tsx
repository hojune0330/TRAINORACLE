...
import { generateMultiAdjustedNextFrameV3FromDraft, type PlanCurrentCheck } from "../../domain/plan-beta-flow"
...
type NextDraft = Extract<ReturnType<typeof generateMultiAdjustedNextFrameV3FromDraft>, { kind: "multi_adjusted_next_frame_v3_draft" }>
export function MultiAdjustedPlanNextFlowV3({ loaded, resolver, readEvidence, onBack, onSaved }: {
  readonly loaded: Extract<PlanBetaStateReadResult, { kind: "multi_adjusted_v3_loaded" }>;
  readonly resolver?: PlanMultiAdjustmentResolverV3;
  readonly readEvidence: () => readonly RetainedMultiAdjustedEvidenceV3[];
  readonly onBack: () => void; readonly onSaved: () => void;
}) {
  ...
  const [entry, setEntry] = React.useState<MultiAdjustmentEditorEntryV3 | null>(null)
  ...
  useActiveContentScroll(entry ? null : draft ? "multi-compare" : "multi-prepare", heading, heading)
  ...
  return <section className="adjusted-next-flow" aria-labelledby={`${id}-title`}>
    ...
    {!draft ? <form onSubmit={event => {
      ...
      try {
        ...
        const result = generateMultiAdjustedNextFrameV3FromDraft({ draft: { ...loaded.state.selection.intake, startDate },
        ...
        if (result.kind === "multi_adjusted_next_frame_v3_draft") setDraft(result)
      ...
      <fieldset><legend>지금 몸 상태는 어떤가요?</legend>
        <label><input type="radio" name={`${id}-check`} checked={check === "NO_KNOWN_RISK"} onChange={() => setCheck("NO_KNOWN_RISK")} />알고 있는 통증이나 이상이 없어요</label>
        <label><input type="radio" name={`${id}-check`} checked={check === "REVIEW_REQUIRED"} onChange={() => setCheck("REVIEW_REQUIRED")} />통증·이상이 있거나 잘 모르겠어요</label>
      </fieldset>
      ...
      <p>현재 사용할 수 있는 기록인지 다시 확인해요. 기록을 고르지 않아도 비교할 수 있어요.</p>
