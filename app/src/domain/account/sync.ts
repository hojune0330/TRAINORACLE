...
import { loadTombstones, tombstonedIds } from "./tombstone"
...
function failed(message: string): SyncOutcome {
  return { ok: false, message, pulled: 0, pushed: 0, deleted: 0, total: loadEntries().length }
}
...
export async function syncNow(userId: string): Promise<SyncOutcome> {
  ...
  // 5. 삭제 전파 — 서버에 남은 사본도 지운다. 여기까지 해야 다른 기기에서도
