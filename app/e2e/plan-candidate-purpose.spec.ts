...
test("explains the support-only difference while keeping the selected purpose identical", async ({ page }, testInfo) => {
  ...
  // Then: both choices preserve the selected purpose and explain only the support-duration difference.
  const comparison = page.getByRole("region", { name: "두 계획 핵심 비교" })
  await expect(comparison.getByText("보조 훈련 시간 범위를 그대로 보여줘요.")).toBeVisible()
  await expect(comparison.getByText("보조 훈련 시간만 짧게 보여줘요.")).toBeVisible()
  await expect(comparison.getByText(/지속 페이스 · LT 1일/u)).toHaveCount(2)
  expect(
