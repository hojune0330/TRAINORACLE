...
test("archives the original in the real plan flow and compares it from its journal", async ({ page }, testInfo) => {
  ...
  await expect.poll(() => opener.evaluate(element => {
    const bounds = element.getBoundingClientRect()
    return element.contains(document.elementFromPoint(bounds.x + bounds.width / 2, bounds.y + bounds.height / 2))
  }), { message: "Restored opener must be reachable above the app navigation" }).toBe(true)
  await page.screenshot({ path: testInfo.outputPath("archived-original-return.png") })
  await opener.click()
