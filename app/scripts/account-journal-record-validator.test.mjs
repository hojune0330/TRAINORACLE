...
for (const [name, codec] of [['source', source], ['server', server]]) {
  test(`${name}: accepts optional mixed exercise records and rejects malformed units without stripping data`, () => {
    const value = record(source.waitingJournal({ exerciseLog: { version: 1, source: 'SELF_REPORTED', components: [
      { id: 'run', kind: 'INTERVALS', name: 'Synthetic intervals', rows: [{ id: 'rep', distanceM: 400, durationSeconds: 80, repetitions: 10, sets: 2, recovery: { kind: 'TIMED', seconds: 60 } }] },
      { id: 'strength', kind: 'STRENGTH', name: 'Synthetic strength', rows: [{ id: 'set', loadKg: 60, repetitions: 5 }] },
    ] } }))
    assert.equal(codec.validateAccountJournalRecord(value), true)
    const changed = structuredClone(value)
    changed.entry.exerciseLog.components[0].rows[0].repetitions = 8
    assert.equal(codec.validateAccountJournalRecordUpdate(value, changed), true)
    changed.entry.exerciseLog.components[0].rows[0].repetitions = 1.5
    assert.equal(codec.validateAccountJournalRecord(changed), false)
    assert.equal(value.entry.exerciseLog.components[0].rows[0].repetitions, 10)
  })
...
test('generated bytes are reproducible and all runtime imports are bundled', async () => {
  ...
  assert.deepEqual(inputs, [
    ...
    'app/src/domain/exercise-log.ts',
