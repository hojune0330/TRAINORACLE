...
6. **감사 보고서 자신이 ID 정규식을 오염시킨다** (2026-08-07 종결 점검에서 실측): D-22 (2)절의 OD-REQ 재수록(`OD-REQ-Dxx-NNN` 15건)과 D-19의 `OI-TEST-DATA-ANONYMIZATION-*` 예시가 `\bOI-[A-Z0-9-]{4,40}\b`·규칙ID 정규식에 매칭돼 **전역 카운트가 224→225, 847→863으로 보였다**. 보고서 제외(`:!reports/review/deepseek-audit/*`) 시 **224/847 정확 일치**를 재확인했다. §9-6 기준값 재현·새 패킷 수행 시 **반드시 자기 산출물을 제외하고 세라** — 특히 OD-REQ 번호(`OD-REQ-Dxx-NNN`)가 규칙ID 정규식과 동형인 것이 함정이다.
