...
| CR-07 | APPROVE_RECORDED_PENDING_INDEPENDENT_REVIEW | Keep separate competition records and count one MAIN placement per competition day with a visible text marker, not a star-only marker. A canonical day-boundary definition remains open. |
...
경계를 모두 확인해야 합니다. 특히 "대회일"의 날짜 경계는 이 기록에서 정본으로
정의하지 않았습니다. 그때까지 `ruleset_accepted`와 `runtime_authority`는 false입니다.
