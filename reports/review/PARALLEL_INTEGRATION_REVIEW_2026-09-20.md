...
- CI run `35517130235`: all four general browser projects passed (164 desktop, 180 mobile, 182 narrow, 160 reduced-motion; existing project-specific skips retained). The separately configured TCX-to-plan test still used pre-redesign entry/heading selectors. It now follows the current entry and explicitly opens prescription details. All nine dedicated file-analysis scenarios passed locally, including exact 222.25-second prescription persistence, fresh-device account hydration, and the +77.75-second comparison; no numerical assertions were removed.
