...
| `BLOCK-SAFETY-BINDING-EVIDENCE` | 이 템플릿에 대한 D9→RVE→Safety Gate→처방 계산 회귀 증거가 없음 | 아래 상속 계약 전 경로와 오래되거나 잘못된 신호의 실패 폐쇄 테스트 |

## 6. D9 / RVE / Safety Gate 상속 계약

이 패킷은 `RULE_SPEC_D1_D9`의 의미를 다시 정의하지 않고 기존 안전 체인을 그대로
상속한다.

| D9 평가 결과 | RVE 저장 상태 | Safety Gate | 상세처방 후보 |
|---|---|---|---|
| `D9_ACTIVE` | `ACTIVE` | 차단, 사람 검토 필요 | 생성·계산·표시 금지 |
| `D9_UNKNOWN` | `UNKNOWN` | 차단, 사람 검토 필요 | 생성·계산·표시 금지 |
| `D9_CLEARED` | `CLEARED` | 이 안전 관문만 통과 | 다른 모든 적격성·검토·구성요소 관문을 통과한 뒤에만 계산 가능 |
| `D9_CLEARED_WITH_ADVISORY` | `CLEARED` | 차단하지 않음, reason code 보존 | `D9_CLEARED`의 하위 의미로만 처리 |

`D9_CLEARED`는 의료적 허가나 안전 보증이 아니다. 좋은 물리치료 정보, 코치 선택,
템플릿 선택, 기록 향상은 `ACTIVE` 또는 `UNKNOWN`을 해제할 수 없다. D9 결과나 RVE
신호가 누락·오래됨·형식 오류·범위 불일치이면 `UNKNOWN`으로 실패 폐쇄하고 계획 생성
전에 차단한다. 사람이나 앱 설정이 Safety Gate를 우회하는 경로도 허용하지 않는다.

이 템플릿을 활성화하는 별도 PR은 위 네 경로와 신호 누락·오래됨·형식 오류를 실제
RVE 및 Safety Gate 입력으로 실행한 회귀 증거를 포함해야 한다.
...
- [ ] `BLOCK-SAFETY-BINDING-EVIDENCE` 해결
- [ ] 오너 검토 승인
- [ ] 코치 검토 승인
- [ ] 스포츠과학 검토 승인
...
4. D9→RVE→Safety Gate를 통과한 같은 요청 안에서만 처방 계산을 허용한다.
5. 승인 레지스트리에 `V2-SEED-05` 한 건만 별도 PR로 추가한다.
6. `FIVE_K`·`EXPERIENCED`·같은 종목 `CURRENT` 앵커에만 후보를 결합한다.
7. 후보 화면에 5회, 1000m, 개인 목표 반복 시간, 150초 조깅, 근거 기록,
   준비운동·정리운동·감량·중단조건을 함께 표시한다.
8. 저장·재열기·다음 주기 연속성까지 같은 버전과 근거를 보존한다.
