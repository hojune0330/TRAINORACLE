...
| Google provider | Disabled | 미연결 |
| Kakao provider | Disabled | 미연결 |
| Phone provider | Disabled | 미연결 |
| Site URL | `http://localhost:3000` | 공개 주소로 수정 필요 |
| Redirect URLs | 없음 | 공개·시험 주소 등록 필요 |
| Database | 최신 원격 migration `0026`, 관련 RLS 정책 9개 | `0027`·`0028` 적용 필요 |
| Google Cloud | TrainOracle 전용 프로젝트 없음 | 새 전용 프로젝트 생성 권장 |
...
이 표는 브라우저에서 보이는 운영 설정을 읽기 전용으로 확인한 결과다. 제공자 연결,
OAuth 키 생성, SMS 발송, SQL 실행 또는 공개 변수 변경을 완료했다고 주장하지 않는다.
...
1. Google Cloud에 TrainOracle 전용 프로젝트를 만든다.
2. 브랜딩 이름을 `TrainOracle`, 지원 이메일을 운영자 승인 주소로 설정한다.
3. 웹 OAuth client를 만들고 위 origin과 Supabase callback만 등록한다.
4. Client ID와 secret은 Supabase Google provider 설정에 직접 넣는다. Git에는 넣지 않는다.
5. Supabase Site URL과 Redirect URLs를 공개 주소로 바꾼다.
6. custom SMTP를 연결하고 `supabase/templates/magic-link.html`과 같은 6자리 코드 템플릿을 적용한다.
7. Google과 이메일을 각각 신규 가입, 로그아웃, 재로그인까지 실측한다.
8. `supabase/tests/account_identity_isolation_rehearsal.sql`을 SQL Editor에서 실행해 마지막 PASS와 rollback을 확인한다.
