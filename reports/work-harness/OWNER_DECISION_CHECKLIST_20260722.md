...
| NOW-04 | service `TrainOracle`; provisional provider `aaclub`; `LEGAL_OPERATOR_DEFERRED` | 서비스명은 `TrainOracle`이다. `aaclub`은 임시 서비스 제공자명이며 확인된 법인명이나 개인정보처리자 법적 명칭으로 취급하지 않는다. 실제 개인정보처리자·사업자·국가·연락처는 미정이다. 서버 수집·결제·공개 출시 전 적격 검토에 실제 운영자 사실을 제공해야 한다. |
...
실제 검토자에게 전달할 역할별 패킷은
[`../review/appt-packets/README.md`](../review/appt-packets/README.md)에 14/14 조립돼 있다.
