...
- 실제 Supabase 계정 A/B 생성·로그인과 서버 RLS 교차 격리는 2026-08-26 스테이징에서 통과했다. 근거는 `reports/operations/ACCOUNT_AB_ISOLATION_RUNTIME_RECEIPT_2026-08-26.md`다.
- 브라우저 프로필 두 개를 사용한 로컬 화면 격리, 로그아웃·재로그인 및 계정 연결 화면의 실제 브라우저 증거는 아직 없다.
...
- 시험 계정·비밀번호·토큰은 저장소에 주입하거나 기록하지 않았고 시험 종료 후 삭제했다.
