...
- 상태: SERVER_SCHEMA_AND_KEY_DEPLOYED_FEATURE_OFF_ACCOUNT_TEST_PENDING
...
9. 계정 상태 검증 2/2, 일지 레코드 검증 81/81, 기본 프론트엔드 빌드를 실행해 통과했다. 이 빌드는 실제 Supabase 구성이나 계정 왕복 증거가 아니다.
10. 로컬 비공개 입력 창에서 받은 운영 DB 비밀번호와 attestation JSON을 메모리에서만 사용해 `account_journal_gateway_keys`에 매개변수 쿼리로 등록했다. 해당 ID의 활성 키는 정확히 1건이고 잘못된 길이의 키는 0건이다. 비밀번호와 키 원문은 파일, Git, 콘솔에 저장하지 않았다.
...
1. 실제 서명 요청으로 Edge 서버와 DB 키가 일치하는지 검증한다.
2. 0035의 레거시 유예 기간은 2026-09-09 운영 적용 시점부터 흐르므로, 활성화 및 이전 일정에서 이를 추적한다.
3. 실제 계정 저장/조회, 사용자 간 격리, 기기 간 조회 및 실패 후 재시도를 검증한다.
4. 검수 후 기능 활성화와 프론트엔드 병합/배포를 수행하고 공개 화면에서 확인한다.

## 미수행

실제 서명 요청 검증, 기능 활성화, 실제 계정 왕복 시험, PR 병합, 프론트엔드 배포는 아직 완료되지 않았다. 함수 ACTIVE, 스키마 적용, DB 서명 키 등록만으로 온라인 저장 기능이 공개됐다고 기록하지 않는다.
