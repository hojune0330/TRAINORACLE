...
2026-09-20 갱신: [운영 적용 기록](PROGRAMMED_FILE_ANALYSIS_ROLLOUT_2026-09-20.md)에서 호환 서버와 0038 적용, 기존 공개 앱 저장·휴지통 확인을 완료했다.
위 YAML은 이 실행문서를 처음 작성한 시점이다. 현재 실행 여부는 날짜별 적용 기록을 우선하며, 새로운 분석 기능은 아직 비활성이다.
...
2026-09-20 공개 순서 보완: 호환 서버와 기존 일지를 먼저 확인한 뒤, 파일 기능을 끈 앱을 먼저 푸시·배포할 수 있다.
새 파일 저장 허용 및 TCX 화면 공개는 그다음 단계다. 나머지 형식의 공개를 푸시 선행조건으로 확대하지 않는다.
