...
- 저장소 `AGENTS.md` §6을 따라 `main` 직접 커밋·푸시로 배포 검사를 실행한다. 필수 CI의 검사 항목이나 배포 선행 조건은 완화하지 않는다.
...
세 번째 실행 `34794567598`은 PC 153건을 통과했고, 모바일은 167건 통과·2건 실패였다.
모바일 전용 포인트 규칙과 기록 없는 계획 안내도 닫힌 도움말을 여는 흐름으로 맞췄다.
스크롤 높이는 기본 닫힘 상태에서 먼저 검증하며 기존 상한을 늘리지 않았다.
변경 전후 39개 소스 파일의 문자열과 e2e 문자열을 대조한 결과, 삭제된 문구를 그대로
참조하는 추가 문자열은 발견하지 못했다. 이는 모든 표시 동작의 검증을 대체하지 않는다.
해당 모바일 재검사는 3건 통과·실패 0·기존 기기 조건에 따른 건너뜀 1건이며,
`release-mobile-gate-repair.json`에 남겼다. 앱 런타임 추가 변경은 없다.

네 번째 실행 `34796879763`은 계약·앱 품질과 PC 153건·모바일 169건을 통과했다.
320px 환경은 170건 통과·1건 실패였다. `분석`을 부분 일치로 찾던 키보드 이동 검사가
새 안내 제목 `분석할 기록이 아직 없어요`까지 찾아 중복된 것이므로, 실제 페이지의
H1을 정확히 찾도록 수정했다. 화면 이동·포커스·가로 넘침 단언은 유지한다.
수정한 320px 검사 1/1과 아직 CI에서 실행되지 못한 줄인 모션 환경 149건을 로컬에서
통과했다. 줄인 모션의 기존 기기 조건부 건너뜀 32건은 통과로 세지 않는다.
`release-narrow-title-repair.json`, `release-final-project-repair.json` 및 e2e 타입 검사로
확인했으며, 이 보완 역시 테스트·보고서만 변경한다.
