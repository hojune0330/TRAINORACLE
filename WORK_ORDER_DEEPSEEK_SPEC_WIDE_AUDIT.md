...
| **F-3** | **문서가 백틱으로 참조하는 유니크 경로 중 상당수(실측 약 12%)가 저장소에서 해석되지 않는다.** 수치는 계측 방법에 따라 달라진다 — v1.0의 "795/112(14%)"는 독립 재현 시 불일치였다. 독립 실측 3회: b91=791/89(11.3%) · d9dc=802/95(11.8%) · a29=775/87(11.2%) | 참고치: **791/89 · 802/95** | D-03 |
...
> ⚠️ **v1.1 정정 (2026-08-06, 독립 다각화 검수 반영):** 아래 수치는 **`grep -rhoE '<NS>-[0-9]{2,4}\b' specs/ | wc -l`** (ID 발생 전체 수)로 재현된다. 이 방법으로 **`FRV2-CONF`는 `specs/` 내부에서 0건이다.** v1.0은 이를 "55"로 적었으나, 실측상 「*`FRV2-CONF` 는 이 표가 정의한 `specs/ 내부` 스코프에 존재하지 않는다*」(전체 발생 92건 = `.omo/evidence` 45 + `.omo/plans` 11 + `reports/target-patch-plans` 26 + `reports/review` 8 + 루트 지시서 2, 유니크 ID 12개). D-09 루프에는 `FRV2-CONF` 가 포함돼 있으나 이는 **충돌 등록부(conflict register) ID**로 `specs/` 내부 0건이며, 저장소 전체 검색 시 md=충돌 등록부·reports 파일 25건, validator=0, code=0이 나온다 — **스펙 규칙 이름공간으로 오해하지 마라.**

```
FA-TC       104      DSB-INV     57      FRV2-CONF   0*
...
* v1.0의 "55"는 재현되지 않는다. specs/ 내부 0건 / 전체 92건(유니크 12).
  전체 분포: .omo/evidence 45 · .omo/plans 11 · reports/target-patch-plans 26
  · reports/review 8 · 루트(이 지시서·형식 문서) 2.
```

### 3.3 문서 metadata 실측 — 스키마가 이원화돼 있다

> ⚠️ **v1.1 정정 (2026-08-06, 독립 다각화 검수 반영):** 아래 수치는 재현 방법이
> **「`specs/reconstruct` + `specs/active` 2폴더에서 `^key[: ]`(줄 시작) 패턴」** 임을 명시한다
> (다른 스코프·패턴을 쓰면 값이 달라진다 — 예: `legacy-reference` 포함 시 `version` 18→24).
> 이 방법으로 **`owner`=15가 아니라 17, `canonical_promotion_allowed`=13이 아니라 12,
> `status` 값 종류=19가 아니라 11** 이다. 나머지 6개는 v1.0과 그대로 일치한다.

```
status: 22개 문서      spec_id: 19개      version: 18개
document_metadata: 17  owner: 17          canonical_promotion_allowed: 12
doc_id: 12
```

**`spec_id`(19) 와 `doc_id`(12) 가 공존한다. 어느 쪽이 정본 규약인지 문서화돼 있지 않다.** → D-04

**`status` 값이 11종이고, 18개 문서는 status가 아예 없다.** 통제 어휘(controlled vocabulary)가 없다. → D-05

D-05는 위 2폴더가 아니라 **`git ls-files 'specs/**/*.md'` 전체**(56건)를 스캔하므로
D-05의 실측값은 이 3.3의 관측값과 **달라도 정상**이다 — 실제 D-05 스코프에서는
status 보유 31 / 부재 25, 값 종류 18종이다. **3.3과 D-05의 값이 다른 것은 모순이 아니라
스코프 차이다. 혼동해 8종이 "사라졌다"고 보고하지 마라** (T-18과 같은 유니크/총참조 혼동 함정).
...
**언제 순진한 방법이 괜찮은가:** 항목이 수백 개 이하이고 각 검사가 작은 파일만 볼 때(D-03의 참조 경로 800여 건 = 3초).
...
| T-4 | **샌드박스 Node v22.23.1 vs CI `node-version: "24"`** (v1.1 정정 — v1.0의 "Node 20"은 부정확) | 샌드박스와 CI의 Node 메이저 버전이 달라 샌드박스에서만 깨지는 실패가 있다 | 실패는 **개수가 아니라 이름으로** 비교 |
...
> ⚠️ **v1.1 정정:** v1.0이 "112건"으로 명시한 것은 계측 방법 의존 수치였다 — 독립 실측 3회 모두 112 미달(b91=89, d9dc=95, a29=87).
> **정지 조건 §9-6은 이 수에 적용되지 않는다.** 아래 명령을 그대로 실행해 **자신의 실측값**을 산출하고 그 값으로 보고하라.
> "재현 실패 = 저장소 변경"으로 오판하지 마라.
...
**목적:** status 값은 스코프에 따라 11종(§3.3: reconstruct+active 2폴더) 또는 **18종(D-05 전체 스코프)** 이고, status가 없는 문서도 있다. 어떤 값이 "승인됨"을 뜻하는지 기계가 알 수 없다.
...
| 6 | §3의 기준 숫자(493 / 55 / 224 등)가 재현되지 않는다 | 정지. 저장소가 바뀌었다. **단 F-3(경로 참조)만은 계측 방법 의존 수치이므로 예외 — D-03이 자기 실측값을 기록한다 (§3.1 및 D-03 v1.1 안내 참조)** |
...
| 6 | 오너 결정 필요 항목(OD-REQ)에 **너의 추천이 없다** | D-22 (2)절 **OD-REQ 블록만** `추천\|권장\|해야 한다` 자가 검색 (지시서 §6 "순서는 권장" 등 일반 용례는 대상 아님 — v1.1) |
...
- [ ] §3 기준 숫자 전부 재현됨   → md=___ (493) / 검증기=___ (55) / CI등록=___ (14)
      ...
      / 경로참조=___ (D-03에서 자기 실측값 기록 — v1.0의 795/112는 계측 의존 참고치. §9-6 적용 제외)
      🔴 하나라도 다르면 §9-6 정지 조건이다. 진행하지 말고 보고하라.
- [ ] 추측 표현 자가 검색 완료   → 검출 건수: ___ (0이어야 한다)
- [ ] 추천 표현 자가 검색 완료 (D-22 (2) OD-REQ 블록 한정 — v1.1)   → 검출 건수: ___ (0이어야 한다)
...
이 저장소에는 문서 493건, 규칙 ID 이름공간 20개 이상, 검증기 55개가 있다. 그런데 **CI는 검증기 14개만 부르고, 규칙 이름공간 중 최소 하나(`DSB-INV`)는 기계 검증이 0건이며, 문서가 가리키는 참조 경로 중 상당수(실측 약 12%, 계측 방법 의존)가 해석되지 않는다.** 즉 **문서와 코드가 갈라져도 아무 알람이 울리지 않는 구간이 넓다.** 선행 감사자는 그 구간 중 한 곳(하루 두 번 훈련 계약)을 깊게 파서 v0.2를 확정했고, 그 과정에서 **자기 자신의 이전 결론이 틀렸음을 발견했다**(C-6). 좁게 깊게 파면 이런 오류를 잡을 수 있지만, **넓은 면적은 여전히 미조사다.** 너의 일은 그 면적을 전부 훑어서, **다음에 깊게 파야 할 곳을 근거와 함께 지목하는 것**이다. 결론을 내리는 것이 아니다.
...
| 버전 | 날짜 | 개정자 | 내용 |
|---|---|---|---|
| 1.0 | 2026-08-06 | Claude (선행 감사자) | 최초 발행 (d9dc4a8). 패킷 24건, 읽기 전용 모드. |
| 1.1 | 2026-08-06 | 저비용 검증 에이전트 (독립 다각화 검수) | 검수 보고(5c323e0)와 실측 3회를 반영한 수치 정정 6건. **사용자(오너)의 최종 권한으로 개정 승인.** |

**1.1 핵심 개정 요약:**

1. **§3.2 — `FRV2-CONF 55` → `specs/` 내부 0건.** 재현 방법(`grep -rhoE '<NS>-[0-9]{2,4}\b' specs/`) 명시. 실분포: 전체 92건(유니크 12) = `.omo/evidence` 45 + `.omo/plans` 11 + `reports/target-patch-plans` 26 + `reports/review` 8 + 루트 2. D-09 루프에 포함돼 있으나 충돌 등록부 ID라 스펙 규칙으로 오해 금지.
2. **§3.3 — `owner 15`→17, `canonical_promotion_allowed 13`→12, `status 종류 19`→11.** 재현 방법(2폴더, `^key[: ]` 줄시작) 명시 + D-05 전체 스코프(18종)와의 차이 안내 추가.
3. **T-4 — "Node 20" → Node v22.23.1(샌드박스) vs `node-version: "24"`(CI).**
4. **F-3 — "795/112" → 계측 의존 참고치로 격하.** 독립 실측 b91=791/89 · d9dc=802/95 · a29=775/87. §9-6 예외 처리, D-03은 자기 실측 우선.
5. **§11/§12 — 자가검색(`추천|권장|해야 한다`) 범위를 D-22 (2) OD-REQ 블록 한정으로 명시.** 지시서 §6 "순서는 권장" 등 일반 용례는 대상에서 제외.
6. **연동 문서 수치 일치화** — `AGENTS.md` §8 표 · `INCOMPLETE_WORK_BACKLOG.md` F-1~F-3 · `reports/review/deepseek-audit/README.md` (본 개정과 함께 갱신).
