...
| 남은 스펙/증거 | 왜 필요한가 | 먼저 해야 할 조건 |
|---|---|---|
| Daily Log binding patches | 일지 입력을 App Bridge, Safety Gate, RVE가 안전하게 소비하게 만들기 위해 | `DAILY_LOG_AND_CHECKIN_SPEC.md` 리뷰와 target patch |
| `DAILY_BRIEF_AND_INBOX_SIGNAL_SPEC.md` | 매일 요약, 코치 알림, AI Inbox 항목을 만들기 위해 | Daily Log 저장 경계 확정 |
| `ANALYSIS_AND_VISUALIZATION_DATA_CONTRACT.md` | 분석 화면과 시각화가 어떤 데이터 구조를 쓰는지 정하기 위해 | Daily Log, Session Classifier, Physio Source Trust 연결 |
| `PLAN_OUTPUT_RATIONALE_PRIVACY_SPEC.md` | 훈련 계획의 이유 설명이 민감정보를 드러내지 않게 하기 위해 | Plan Generator와 App Bridge privacy 경계 확인 |
| `MICROCYCLE_AND_CALENDAR_MAPPING_SPEC.md` | 9.5-day cycle, 달력, 경기일 기준 표기를 헷갈리지 않게 하기 위해 | `RULE_SPEC_D1_D9.D-*`와 `CYCLE_DAY.D-*` namespace 분리 유지 |
...
5. Daily Brief, AI Inbox, Analysis, Calendar 쪽 제품화 스펙 작성.
...
Remaining productization SPECs:
