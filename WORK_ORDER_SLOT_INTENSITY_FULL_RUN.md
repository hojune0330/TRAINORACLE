...
- 🔴 `recoverySecondSessionDays()`의 `limit = 2`는 **짝 세션과 공유하는 예산이 된다.**
  짝 세션도 `RECOVERY_INTENT`이므로 `DSB-INV-005` 상한(9·9.5·10일 프레임 **2건**)에
  같이 들어간다. 고강도 날의 짝을 먼저 채우고 **남은 몫만** 회복 PM에 쓴다 —
  **반드시 §3.1을 읽어라. 안 읽으면 71% 조합이 사양 상한을 넘는다.**
...
> **📌 이 절의 실측 이력 (읽고 시작해라).** 이 절은 두 번 정정됐다.
> 1차: 필드명을 `sessionDay`로 잘못 읽어 숫자가 뒤섞였다.
> 2차: 짝 세션의 의도를 `BASE_INTENT`로 착각해 *"상한 밖이라 위반이 없다"* 고
> 잘못 정정했다. **둘 다 틀렸다.** 아래는 3차 실측이며, 실제 필드명은 `day`,
> 짝 세션의 의도는 C3A `:208`에 따라 **`RECOVERY_INTENT`** 다. 따라서
> **`DSB-INV-005` 상한에 포함되고, 상한 초과는 사실이다.**
> 재현 방법은 이 절 끝의 「실측 재현 절차」를 따라라.

위 `counterpartSessions()`는 **고강도 날마다 세션 한 건을 더 만든다.**
그 세션은 C3A `:208`에서 `easyTrainingSession(day, other, ranges.recoverySupport,
"RECOVERY_INTENT")` 이다 — 즉 **`RECOVERY_INTENT`** 다. 그리고
`recoverySecondSessionDays()`가 만드는 회복 PM도 `RECOVERY_INTENT`다.
**두 장치가 같은 상한(`DSB-INV-005`)을 나눠 쓰는데, 서로를 모른다.**

**실측** (`generatePlanFromDraft(draft, "NO_KNOWN_RISK")`,
`RECOVERY_PM_ALLOWED`, BALANCED, 9.5일 프레임, 420개 조합 전수):

| 조합 수 | 현재 PM 회복 건수 | 고강도 날 | 현재 하루2회 날 | S-2 후 PM 회복 건수 | S-2 후 하루2회 날 | 판정 |
|---:|---:|---:|---:|---:|---:|---|
| 60 | 2 | 1 | 2 | **3** | 3 | ⚠ 상한 2 초과 |
| 20 | 2 | 1 | 2 | **3** | 3 | ⚠ 상한 2 초과 |
| 20 | 2 | 1 | 2 | **3** | 3 | ⚠ 상한 2 초과 |
| 20 | 2 | 1 | 2 | **3** | 3 | ⚠ 상한 2 초과 |
| 20 | 2 | 1 | 2 | **3** | 3 | ⚠ 상한 2 초과 |
| 40 | 2 | 2 | 2 | **4** | 4 | ⚠ 상한 2 초과 |
| 40 | 2 | 2 | 2 | **4** | 4 | ⚠ 상한 2 초과 |
| 40 | 2 | 2 | 2 | **4** | 4 | ⚠ 상한 2 초과 |
| 40 | 2 | 2 | 2 | **4** | 4 | ⚠ 상한 2 초과 |
| 60 | 2 | 0 | 2 | 2 | 2 | 정상 (고강도 없음) |
| 60 | 0 | 0 | 0 | 0 | 0 | 정상 (회복 의도 등) |

**420조합 중 300건(71%)이 `DSB-INV-005` 상한 2건을 넘는다.**
(고강도 1일 → 3건 / 고강도 2일 → 4건)

실측 최악 사례 (`MIDDLE_DISTANCE`/`DEVELOPING`/가능 4일/`LT_INTENT`):

```
현재 (실측): 훈련 6회 / 220분 / 하루2회 날 2일 / 고강도 날 [4, 10]
S-2 후     : 훈련 8회 / 270분 / 하루2회 날 4일 / PM 회복 4건  ← 상한 2 초과
```

밴드별 실측 (`MIDDLE_DISTANCE`/`LT_INTENT`, 각 범위의 **상한값** 합산):

| 밴드 | 가능일수 | 현재 | S-2 후 | 증가율 |
|---|---|---|---|---:|
| NEW_TO_RUNNING | 4일 | 6회 175분 | 7회 195분 | +11% |
| NEW_TO_RUNNING | 매일 | 12회 385분 | 13회 405분 | +5% |
| DEVELOPING | 4일 | 6회 220분 | 8회 270분 | **+23%** |
| DEVELOPING | 매일 | 12회 490분 | 14회 540분 | +10% |
| EXPERIENCED | 4일 | 6회 280분 | 8회 340분 | **+21%** |
| EXPERIENCED | 매일 | 12회 640분 | 14회 700분 | +9% |

**가장 위험한 건 "매일"이 아니라 "가능일수 4일"이다.** 가능한 날이 적으면
고강도 날이 전체에서 차지하는 비중이 커져서, 그 4일이 **전부** 하루 두 번이 된다.
(질문은 *"이번 계획에서 운동할 수 있는 날은 며칠인가요?"* 이고 프레임은 9.5일이다.
9.5일 중 4일이며 **"일주일에 4일"이 아니다.** `spreadTrainingDays(4) = [1,4,7,10]`.)

#### 이건 사양 상한을 넘는다 — 그리고 이 상한은 살아 있다

`DOUBLE_SESSION_BETA_SAFETY_CONTRACT.md` **v0.2** `DSB-INV-005`:
*"Balanced는 7일 프레임에 PM `RECOVERY_INTENT` 최대 1건, 9·9.5·10일 프레임에
최대 2건. 짝 세션이 `RECOVERY_INTENT`로 만들어지면 그것도 이 상한에 포함된다."*
실측 프레임은 전부 `9.5`일 → **상한 2건인데 3~4건이 된다.**

`DSB-INV-002`/`003`은 오너 결정(OD-SLOT-1·2·8)에 따라 **v0.2에서 은퇴했다**(§4.6b).
**하지만 `DSB-INV-005`는 은퇴하지 않았고 v0.2에서 오히려 범위가 명확해졌다.**
오너 결정 OD-SLOT-8(*"하루 2회 매일 하는 것도 문제가 크게 되지 않아"*)은
**하루 두 번의 허용 여부**에 대한 판단이지, **회복 세션 건수 상한을 없애라**는
지시가 아니다. 두 축을 섞지 마라:

| 축 | 누가 정했나 | 지금 상태 |
|---|---|---|
| 하루 두 번을 **허용**하는가 | OD-SLOT-8 | **허용** (단 `DSB-INV-009` 고지·수정 흐름 조건) |
| 하루 안에 고강도를 **몇 번** 두는가 | OD-SLOT-2·3 | 기본 1회, 2회는 명시 지정 시만 |
| PM **회복 세션**을 프레임에 **몇 건** 두는가 | `DSB-INV-005` (오너 결정 없음) | **상한 유지** — 7일 1건 / 9·9.5·10일 2건 |

세 번째 축은 **오너가 판단한 적이 없다.** 그래서 사양 상한을 그대로 지킨다.

#### 그래서 S-2에서 이렇게 해라 — 총량을 보존한다

```
짝 세션과 순수 회복 PM은 하나의 예산을 나눠 쓴다.
예산 = DSB-INV-005 상한 (9·9.5·10일 프레임 2건 / 7일 프레임 1건)

1. 먼저 고강도 날의 짝 세션을 배정한다 (우선권)
2. 남은 예산만큼만 recoverySecondSessionDays()가 회복 PM을 만든다
3. 남은 예산이 0이면 순수 회복 PM은 0건이다
```

- 상한 숫자: **7일 프레임 1 / 9·9.5·10일 프레임 2** (`DSB-INV-005` 숫자 그대로)
- 🔴 **세는 단위는 `RECOVERY_INTENT` PM "세션 건수"다.** "하루 두 번인 날의
  수"가 아니다. (v0.2 `DSB-INV-005`가 단위를 명시했다. 이전 판 이 지시서가
  "날의 수"라고 적은 건 틀렸다.) 현재 생성기에서는 짝 세션도 순수 회복 PM도
  각각 하루 1건이므로 두 숫자가 우연히 같지만, **같다고 가정한 코드를 쓰지 마라.**
- **고강도 날의 짝 세션이 우선이다.** 줄이는 쪽은
  `recoverySecondSessionDays()`가 만든 순수 회복 PM이다
  (고강도 날의 짝은 고강도 자체를 지우지 않고는 못 줄인다)
- 결과적으로 `limit = 2`는 **"고강도 날에 적용하지 않는다"가 아니라
  "고강도 날의 짝을 먼저 채우고 남은 몫만 회복 PM에 쓴다"** 가 된다.
  **C3A `:217`의 원문은 철회됐다** (그 문서에도 철회 표시를 넣어 뒀다)
- **대안 검토 (선택 가능):** 짝 세션을 `RECOVERY_INTENT`가 아니라
  `BASE_INTENT`로 만들면 `DSB-INV-005` 상한 밖이 되어 예산을 나눌 필요가 없다.
  **하지만 이 길을 임의로 고르지 마라.** OD-SLOT-7은 권장을 "휴식 또는 회복
  운동"으로 정했고 C3A `:195`가 그래서 `RECOVERY_INTENT`를 고른 것이다.
  `BASE_INTENT`로 바꾸면 고강도 날 뒤에 RPE 3-4를 붙이는 것이 되어 **오히려
  부담이 커진다.** 바꾸고 싶으면 **§9-14로 멈추고 물어라.**

**최악 사례가 이렇게 바뀐다:** 고강도 날 2일(DAY4·DAY10)의 짝이 예산 2를 다 쓰므로
DAY1·DAY7의 회복 PM은 만들지 않는다
→ PM 회복 **2건 유지**, 하루 2회 날 2일 유지, 훈련 6회 220분 유지.

#### 고정해야 하는 테스트 (impl, S-2 커밋에 포함)

| ID | 무엇을 고정 | 단위 |
|---|---|---|
| G-20 | 9.5일 프레임 BALANCED에서 **PM `RECOVERY_INTENT` 세션 ≤ 2건** — 위 최악 조합(`MIDDLE_DISTANCE`/`DEVELOPING`/4일/`LT_INTENT`)으로 | 세션 건수 |
| G-21 | 고강도 날이 2일이면 **순수 회복 PM은 0건** (짝이 예산을 다 씀) | 세션 건수 |
| G-22 | 🔴 **대조군.** 고강도 날이 0일이면(`BASE_INTENT` 등) 회복 PM은 **2건 그대로** — 회귀 없음 | 세션 건수 |
| G-23 | `SINGLE_SESSION_ONLY`면 **PM 세션 0건** (`DSB-INV-001`) | 세션 건수 |
| G-24 | 하루 세션 수는 **어떤 조합에서도 2를 넘지 않는다** (`DSB-INV-004`) | 하루 세션 수 |

**G-22가 비공허성 대조군이다.** G-20·G-21·G-23만 쓰면 **"짝 세션 기능을 아예
안 만들기"로도 전부 통과한다.** §4.7과 같은 함정이다. G-22는 "줄이기만 하면
통과"를 막고, 추가로 **짝 세션이 실제로 생기는 것을 단정하는 테스트도 하나
넣어라** (고강도 날의 반대 슬롯에 세션이 존재한다).

#### 실측 재현 절차 (숫자를 못 믿겠으면 직접 돌려라)

```
1. app/src/ 아래에 임시 테스트를 만든다
2. import { generatePlanFromDraft } from "./domain/plan-beta-flow"
3. generatePlanFromDraft(draft, "NO_KNOWN_RISK")  ← 2번째 인자 필수
4. 후보 배열은 out.generated.candidates 다 (out.candidates 아님)
5. 후보 구분은 c.kind === "BALANCED" 다 (c.candidateKind 아님)
6. 🔴 세션의 날짜 필드는 s.day 다. s.sessionDay 는 undefined 다
7. 의도 값은 GLY_INTENT 다. GLYCOLYTIC_INTENT 아님
8. 다 끝나면 임시 파일을 rm 하고 git status로 잔여물 0을 확인한다
```

이 8줄은 **내가 실제로 틀린 순서 그대로**다. 4·5·6번을 틀리면 숫자가 조용히
0이나 엉뚱한 값이 되고, **테스트는 통과한다.** 값이 전부 0이거나 전부 같으면
버그를 의심해라.

> **🛑 예산 규칙을 넣었는데도 PM `RECOVERY_INTENT`가 상한을 넘으면 §9-14로 멈춰라.**
...
> **✅ 사양은 이미 개정됐다 (2026-08-06).**
> `specs/reconstruct/DOUBLE_SESSION_BETA_SAFETY_CONTRACT.md`는 **v0.2**이고,
> 옛 v0.1 규칙은 은퇴하고 새 규칙으로 교체됐다. 은퇴 원문은 그 문서 **§10 변경
> 이력**에만 역사 기록으로 남아 있다.
> **이제 사양과 코드가 서로 반대말을 하지 않는다.** 이전 판 이 지시서가
> *"사양을 폐기하고 진행한다"* 라고 한 것은 더 이상 맞지 않다 — **사양을 따르면 된다.**
> 사양 문서를 다시 고칠 필요도 없다.

S-3이 바꾸는 검사와 v0.2 사양의 대응은 이렇다:

| v0.2 불변식 | 요지 | 현재 코드 | S-3 후 |
|---|---|---|---|
| `DSB-INV-001` 명시 동의 | 하루 2세션은 `RECOVERY_PM_ALLOWED` 있을 때만 | `:92-94` ("PM 있으면") | **주체 변경** — "그 날 세션이 2개면"으로. 검사는 **없애지 않는다** |
| `DSB-INV-002` 두 번째 세션 모양 | `REST`/`EASY`/`QUALITY` 모두 가능. 휴식·가벼움은 **권장**이지 강제 아님 | `:83-91` (PM은 회복만) | **삭제** — v0.2가 이 제약을 은퇴시켰다 |
| `DSB-INV-003` 같은 날 고강도 수 | **기본 1회.** 2회는 명시 지정 + `DSB-INV-009` 만족 시만 | `:108-113` (PM+QUALITY 금지) | **조건 교체** — "하루 `QUALITY` 2개 이상이면 거부"로 |
| `DSB-INV-004` 하루 상한 | 하루 최대 2세션, `(day,slot)` 유일 | `:76-79`, `:104-107` | **유지** (S-6이 leaf에도 추가) |
| `DSB-INV-005` 회복 세션 상한 | PM `RECOVERY_INTENT` 7일 1건 / 9·9.5·10일 2건 | 관문에 **없음** (생성기 `limit=2`만) | **관문에 넣지 않는다.** 생성기(§3.1 예산 규칙)가 지킨다 |
| `DSB-INV-007` 보충 금지 | 놓친 세션을 옮기거나 복제하지 않음 | 해당 코드 없음 | **유지** (건드릴 것 없음) |
| `DSB-INV-008` RPE 전용 | 처방은 `RPE_TIME_RANGE`만 | 자동 만족 | **유지** — 아래 참조 |
| `DSB-INV-009` 특별한 날 고지·수정 | 하루 고강도 2회는 고지 + 수정 흐름 있을 때만 | 흐름 **없음** | 🔴 **그래서 하루 고강도 2회는 이번에 열지 않는다.** 아래 참조 |

🔴 **`DSB-INV-005`를 저장 관문에 넣지 마라.** 상한을 지키는 책임은 **생성기**에
있다(§3.1). 관문에도 넣으면, 예산 계산이 어긋난 순간 사용자에게
`PLAN_STORAGE_WRITE_FAILED`(*"계획을 이 기기에 저장하지 못했어요"*)가 뜬다 —
사용자가 고칠 방법이 없는 실패다. 생성기 쪽에서 막는 게 맞다.

#### 🔴 `DSB-INV-009` — 하루 고강도 2회는 **이번 범위에서 열지 않는다**

v0.2 `DSB-INV-009`(OD-SLOT-8)는 하루 고강도 2회 같은 특별한 날을 허용하되
**두 조건을 모두** 요구한다: (a) 화면이 그 날이 특별하다고 **알려준다**,
(b) 사용자가 **검토·수정·삭제할 수 있는 흐름**이 있다.

**실측 (2026-08-06):** 그 흐름은 **없다.**
`ls app/src/screens/plan-beta/` 에 수정 화면 없음,
`grep -rn "movePlanSession\|editSession" app/src` **0건.**

→ 그래서 §4.4의 결론은 그대로다: **하루 `QUALITY` 2개 이상은 계속 거부한다.**
그 흐름은 백로그 `B-17`(OD-SLOT-6)이고 §12에서 이번 범위 밖으로 뺐다.
**"명시 지정" 입구를 이번에 만들지 마라.** 만들면 (a)·(b) 없이 특별한 날이
생겨서 `DSB-INV-009` 위반이다.

**보고서에 위 표를 그대로 옮겨 적어라** — 나중에 이 코드를 보는 사람이
"사양과 다른데?" 하고 되돌리는 것을 막는 장치다. 단 이제는 **"사양대로 했다"**
라고 쓸 수 있다.

⚠️ **`DSB-INV-*`는 어떤 CI 검증기도 확인하지 않는다** (사양 §0). 문서에 적혀
있으니 지켜진다고 가정하지 마라. 위 표의 각 행마다 **테스트를 붙여라**(§4.6).

#### 🔴 DSB-INV-008은 은퇴하지 않았다 — 오후 고강도에도 적용된다

v0.2 `DSB-INV-008`: *"**Any beta session, AM or PM**, may show only duration
range, RPE range, intent, and plain-language guidance. It must not show derived
pace, repetitions, distance, or recovery intervals."*
(v0.2에서 주체가 "PM output"에서 **"모든 베타 세션"**으로 넓어졌다 — 오후 고강도가
가능해졌으니 당연히 그쪽에도 걸린다.)
...
#### 🔴 사양 §6 표기 요구 — v0.2에서 교체됐다

v0.1 §6은 *"A PM session must be called an afternoon recovery session"* 이었다.
**오후에 고강도가 갈 수 있으므로 이 요구는 성립하지 않아 은퇴했다.**
v0.2 §6은 대신 이렇게 정한다:

- PM 세션의 이름·안내는 **그 세션의 role·intent에서 나와야 한다.** 슬롯이 PM이라는
  사실에서 나오면 안 된다
- 어떤 화면·용어집도 *"오후는 항상 회복"*, *"오후는 항상 RPE 1~2"*,
  *"오후는 고강도가 아니다"* 라고 **약속하면 안 된다** (v0.1에선 사실이었고
  v0.2에선 거짓이다) → 이게 **S-4가 고치는 3개소**의 근거다
- 놓친 훈련 보충 아님(`DSB-INV-007`), 날짜 캘린더 아님, 의료 판단 아님은 **그대로 유효**
...
| 14 | S-2 결과로 한 프레임의 **PM `RECOVERY_INTENT` 세션이 `DSB-INV-005` 상한(2건)을 넘는다**, 또는 그걸 피하려고 짝 세션의 의도를 `BASE_INTENT` 등으로 **바꾸고 싶다** | 훈련 안전 문제. §3.1의 「대안 검토」를 읽고 **묻고 나서** 결정해라 |
...
14. **프레임 훈련량 실측표** (§3.1) — 예산 규칙 적용 **전/후**를 같은 조합으로
    비교해서 적어라. 최소 3개 열: **PM `RECOVERY_INTENT` 세션 건수** /
    하루 2회 날 수 / 총 훈련 분(각 범위의 상한값 합). "넘지 않는다"는 말만
    쓰지 마라. 재현 절차는 §3.1의 「실측 재현 절차」 8줄을 그대로 따라라
    (`out.generated.candidates`, `c.kind`, `s.day` — 이 3개를 틀리면 숫자가
    조용히 0이 되고 테스트는 통과한다)
...
| 🔴 S-2에서 **공유 예산 규칙을 안 넣음** | PM 회복이 2건→3~4건. **420조합 중 300건(71%)이 `DSB-INV-005` 상한 초과.** §3.1 |
| 🔴 실측할 때 `s.sessionDay`를 읽음 | **필드명은 `s.day`다.** `undefined`가 나오는데 테스트는 통과한다. 내가 실제로 틀렸다. §3.1 재현 절차 |
| 🔴 실측할 때 `out.candidates`·`c.candidateKind`를 읽음 | 각각 `out.generated.candidates`·`c.kind`다. 조합 0건이 되는데 눈치채기 어렵다. §3.1 |
| 🔴 짝 세션이 `DSB-INV-005` 상한 밖이라고 가정 | **`RECOVERY_INTENT`라서 안에 들어간다**(C3A `:208`). 이 착각이 71% 초과를 놓치게 만든다 |
