...
**라이브 웹 앱: [TrainOracle 훈련일지 열기](https://hojune0330.github.io/TRAINORACLE/)**

지금 바로 회원가입 없이 훈련 후 일지, 하루 마무리, 경기 기록을 남기고 거리·기분·통증 추이를 볼 수 있습니다. 입력하지 않은 값은 분석에서 제외하며, 기록은 현재 사용 중인 브라우저와 기기에만 저장됩니다.

| 지금 바로 되는 것 | 아직 준비 중인 것 |
|---|---|
| 로컬 일지 작성·조회·삭제 | 계정 연동과 기기 간 동기화 |
| 실제 입력만 반영한 거리·기분·통증 추이 | 코치 연결과 코치 승인 |
| 메모 제외 기본 내보내기, 동의 후 전체 백업 | 안전 게이트를 통과한 훈련계획 생성 |
| 모바일·데스크톱 웹 앱, PWA 설치 | 유료 플랜과 결제 |

브라우저 데이터 삭제나 기기 변경 전에는 앱의 내보내기 기능으로 백업해 주세요. 훈련계획에 관심이 있다면 현재 화면의 `서비스 준비 중` 안내를 확인하고, 문의는 저장소의 [GitHub Issues](https://github.com/hojune0330/TRAINORACLE/issues)에 남길 수 있습니다.
...
중요한 약속: 아직 완성 앱, 정본 승인, production 배포, Formation runtime evidence 단계는 아닙니다. 기존 D9 evaluator 로그는 존재하지만 Formation 또는 전체 target coverage 증거가 아니며 별도 평가가 필요합니다.
