...
>
> **AI 작업자는 이어서 [`AGENTS.md`](./AGENTS.md)를 읽으십시오.** 훈련·페이스·
...
훈련계획 베타는 회원가입 없이 후보를 만들고, 원하는 후보를 선택해 진행 상태를 남길 수 있습니다. 현재 계획과 기록은 사용 중인 브라우저와 기기에만 저장됩니다. 숙련 선수가 현재 동일 종목 기록을 직접 고르면 800m·1500m·3000m·5000m의 승인된 상세 훈련과 개인 목표 시간을 함께 볼 수 있습니다.

| 이번 베타 제공 범위 | 아직 준비 중인 것 |
|---|---|
| 로컬 일지 작성·조회·삭제 | 계정 연동과 기기 간 동기화 |
| 실제 입력만 반영한 거리·기분·통증 추이 | 코치 연결과 원격 코치 승인 |
| 안전 확인 후 7일·9일·10일 후보 2개와 4개 지원 종목의 상세 세션 생성 | 일지 결과를 반영하는 정식 적응형 계획과 코치 규칙 |
| 후보 선택, 진행 기록, 다음 주기 구조화 연결 | 유료 플랜과 결제 |
| 직접 확인한 방문 날짜당 1P, 적격 훈련·회복 기록 날짜당 4P의 로컬 오라클 포인트와 꾸미기 | 계정 기반 포인트 동기화와 추가 꾸미기 카탈로그 |
| 메모 제외 기본 내보내기, 동의 후 전체 백업 | 의료 판단 또는 부상 복귀 판정 |
| 모바일·데스크톱 웹 앱, PWA 설치 | 계정 기반 장기 보관 |

브라우저 데이터 삭제나 기기 변경 전에는 앱의 내보내기 기능으로 백업해 주세요. 상세 페이스는 사용자가 고른 현재 동일 종목 기록과 승인된 템플릿으로만 계산합니다. 기록이 없거나 오래됐거나 다른 종목이면 RPE 중심 후보로 돌아가며, 통증·부상·몸 이상이 있거나 상태를 평가할 수 없으면 계획을 만들지 않습니다. 최근 일지 수치는 아직 다음 계획의 훈련량을 자동 조정하지 않습니다. 문의와 베타 의견은 공개 저장소가 아니라 [앱 안의 문의 게시판](https://hojune0330.github.io/TRAINORACLE/?feedback=1)에 남깁니다.
...
- **현재 개인 맞춤 훈련계획 지원표:** [`PERSONALIZED_PRESCRIPTION_RELEASE_2026-08-18.md`](./reports/implementation/PERSONALIZED_PRESCRIPTION_RELEASE_2026-08-18.md). 지원 종목, 기록 조건, 상세 세션, 폴백과 검증 근거를 한곳에 정리했습니다.
...
- 추론·자료량별 3단계 작업 선별: [`reports/work-harness/README.md`](./reports/work-harness/README.md)
...
- Wave D downstream binding patch 상태: [`SPEC_WAVED_BINDING_PATCH_REPORT.md`](./SPEC_WAVED_BINDING_PATCH_REPORT.md)
- Visual SPEC status dashboard: [`dashboard/index.html`](./dashboard/index.html)
...
| `ACCOUNT_AUTHENTICATION_AND_IDENTITY_LINKING_SPEC.md` | Google·이메일 확인 링크, 가입 전 나이·약관, Supabase Auth | 비밀번호 없는 1차 가입·로그인과 14세 미만 로컬 사용 경계 |
...
중요한 약속: 이번 공개 베타는 완성 앱, 정본 승인, Formation runtime evidence 단계가 아닙니다. 기존 D9 evaluator 로그는 존재하지만 Formation 또는 전체 target coverage 증거가 아니며 별도 평가가 필요합니다.

> Wave B Safety Gate target patch status: [`SPEC_WAVEB_SAFETY_GATE_PATCH_REPORT.md`](./SPEC_WAVEB_SAFETY_GATE_PATCH_REPORT.md). Wave 1 physio target patch status: [`SPEC_WAVE1_PHYSIO_PATCH_REPORT.md`](./SPEC_WAVE1_PHYSIO_PATCH_REPORT.md). Account/backend planning context: [`ACCOUNT_FEDERATION_DECISION.md`](./ACCOUNT_FEDERATION_DECISION.md), [`ATHLETETIME_INTEGRATION_REVIEW.md`](./ATHLETETIME_INTEGRATION_REVIEW.md), [`LAUNCH_BACKEND_AND_ACCOUNT_PLAN.md`](./LAUNCH_BACKEND_AND_ACCOUNT_PLAN.md). Next patch order: [`SPEC_TARGET_PATCH_MATRIX.md`](./SPEC_TARGET_PATCH_MATRIX.md). Review packet: [`SPEC_REVIEW_PACKET.md`](./SPEC_REVIEW_PACKET.md). Target patch readiness: [`SPEC_TARGET_PATCH_READINESS.md`](./SPEC_TARGET_PATCH_READINESS.md). File-truth guardrails: [`SPEC_FILE_TRUTH_GUARD.md`](./SPEC_FILE_TRUTH_GUARD.md).
...
> 디자인 / UI 구현 작업을 이어받을 때는 [`HANDOFF.md`](./HANDOFF.md)를 읽고, 그 다음 [`PHILOSOPHY.md`](./PHILOSOPHY.md)를 읽으세요. 단, PHILOSOPHY의 9.5일 고정 D-5/수치 처방 절은 현재 Formation 실행 권위가 아닌 legacy product philosophy이며 충돌은 multiperspective review에서 추적합니다.
...
**TRAINORACLE**은 800m부터 장거리까지 달리는 선수를 위한 **AI 코치 시스템**입니다.

> **"너의 데이터를 모두 줘봐. AI 코치 장호준이 분석해서 해결해줄게."**

사용자 ↔ AI 코치 장호준의 **1:1 관계**가 중심. 사용자의 훈련 데이터·기록·목표·컨디션을 가져와서, 그 사람만의 맥락을 이해하고 지속적인 코칭을 제공합니다.

> 이 문단은 legacy 제품 정체성 문구입니다. 첫 Formation pilot의 실행 권한은
> 검증된 연결 인간 코치에게 있으며, AI는 후보/근거 제안 역할입니다. 두 표현의
> 최종 제품 정리는 `OI-FA-PRODUCT-PROJECTION-001`에서 결정합니다.
...
├── TRAINING_PLAN_BLUEPRINT_MULTIPERSPECTIVE_REVIEW.md ← 9개 관점 검증·Mermaid 청사진
├── SPEC_WAVEB_SAFETY_GATE_PATCH_REPORT.md ← Plan Generator Safety Gate target patch 결과
├── HANDOFF.md                        ← 개발 에이전트용 첫 프롬프트
├── PHILOSOPHY.md                     ← 제품 철학·UX 원칙; §4 훈련 처방은 Formation 비정본 legacy 입력
