...
f0_f9_implementation_commit: 96bd469ef74cc6030eb8ee611ffc9f036753fcc4
...
- 공동기획 응답 기준: `4f85946` (PR #56)
- F0-f-9 구현 기준: `96bd469` (PR #58)
- 열린 PR: 0건
- 열린 작업 이슈: `#42`, `#21`
...
Fable은 이 사실을 `LogEntry.tsx`와 `aggregates.ts`에서 독립 확인하고 PR #56으로 ORDER_008 v3와 F0-f-9에 반영했다. 이어 PR #58에서 시작값을 0(미선택·미기록)으로 바꾸고, 0을 화면에서 `—` 또는 `미기록`으로 표시하도록 구현했다. PR #58에는 typecheck, build, gh-pages bundle, `[JSTORE]`·`[HOMEJ]` 콘솔 확인이 기록돼 있다. 정식 provenance metadata와 legacy 판독은 여전히 ORDER_008 Task A의 후속 계약이다.
...
- 빈 저장과 implicit default의 즉시 오염 차단은 PR #58로 구현됨
- EXPLICIT/DERIVED/MISSING metadata와 legacy 판독은 ORDER_008 Task A에서 확정
...
| D2 데이터 진실성 | provenance·eligibility 필수 | ORDER_008 v3 수용 + PR #58 즉시 오염 차단 | implicit default 분석 제외 | metadata 이름·legacy·IMPORTED/DEMO 범위 | PENDING |
...
3. PR #58의 즉시 조치 위에 ORDER_008 v3와 ORDER_009 v2 계약을 완성한다.
