...
function Home({ variant = 'A', encoding = 'dot-code', showAI = true, onWriteLog, onOpenDay }) {
  if (variant === 'B') return <HomeSparkline onWriteLog={onWriteLog} onOpenDay={onOpenDay} showAI={showAI} encoding={encoding} />;
  if (variant === 'C') return <HomeRecall onWriteLog={onWriteLog} onOpenDay={onOpenDay} showAI={showAI} encoding={encoding} />;
  return <HomeIndexCard onWriteLog={onWriteLog} onOpenDay={onOpenDay} showAI={showAI} encoding={encoding} />;
}

// ───────── A. Index Card top (most journal-like) ─────────
function HomeIndexCard({ onWriteLog, onOpenDay, showAI, encoding }) {
  return (
...
function HomeSparkline({ onWriteLog, onOpenDay, showAI }) {
        ...
        <div style={{ borderTop: '1px solid var(--ink)', borderBottom: '1px solid var(--ink)', margin: '0 20px' }}>
          {[
            ...
            <div key={i} style={{
              ...
              <Delta value={r.d} suffix={r.dsuf} invert={r.invert} />
            </div>
          ))}
        </div>
      </div>
...
function HomeRecall({ onWriteLog, onOpenDay, showAI }) {
  return (
    <div style={{ paddingBottom: 90 }}>
