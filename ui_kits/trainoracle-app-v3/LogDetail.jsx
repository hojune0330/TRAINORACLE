...
function LogDetail({ date = '2026-06-21', variant = 'A', onBack }) {
  if (variant === 'B') return <LogDetailDashboard date={date} onBack={onBack} />;
  return <LogDetailJournal date={date} onBack={onBack} />;
}

// ───────── A. Journal-page (paper-feel) ─────────
function LogDetailJournal({ date, onBack }) {
  return (
    <div style={{ paddingBottom: 40 }} className="paper-grid">
      ...
      <div style={{ padding: '14px 20px 0' }}>
        <IndexCard date="2026·06·21" dow="SUN · 16°C 맑음" cycleDay="C7 · D-5 ★ MAIN" season="2026 spring" />
      </div>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--line)' }}>
          {[
            ...
            <div key={i} style={{
              ...
              <span>
                {extra?.v != null && <Delta value={extra.v} suffix="kg" invert={extra.invert} />}
...
function LogDetailDashboard({ date, onBack }) {
  return (
    <div style={{ paddingBottom: 30 }}>
      <TopBar2 onBack={onBack}>일지 · 데이터 뷰</TopBar2>
      <div style={{ padding: '14px 20px 0' }}>
        <IndexCard date="2026·06·21" dow="SUN" cycleDay="C7 · D-5 ★" />
      </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', borderTop: '1px solid var(--ink)', borderBottom: '1px solid var(--ink)' }}>
          {[
            ...
            <div key={i} style={{
              ...
              {ex?.delta != null && <Delta value={ex.delta} suffix={u} invert={ex.invert} />}
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ padding: '20px 20px 0' }}>
        <div style={{ fontFamily: 'var(--mono)', fontSize: 10.5, color: 'var(--ink-2)', lineHeight: 1.5, padding: 12, background: 'var(--surface-2)' }}>
          <b style={{ color: 'var(--ink)' }}>드리프트</b> rep 4→5에서 +4", 5→6에서 +1". 권고 ±3" 초과 — <b style={{ color: 'var(--warn)' }}>R-7 경고</b>.
        </div>
      </div>
    </div>
  );
...
function TopBar2({ onBack, children, action }) {
  return (
    <div style={{
...
Object.assign(window, { LogDetail });
